/*
** EPITECH PROJECT, 2022
** Enum
** File description:
** Enum
*/

#pragma once

#define NB_MAX_PLAYERS_PER_ROOM 4
#define ROOM_ID_SIZE 8
#define DISCONNECTION_TIME 1500

namespace ecs
{
    enum Scenes { ROOM1, GAME1, ROOM2, GAME2, ROOM3, GAME3, ROOM4, GAME4,
                PUBLICROOM, PRIVATEROOM, LISTROOM, JOINROOMBYID, QUITROOM,
                MENU, SETTINGS, WIN, LOOSE, GAME, TYPEPSEUDO, HOWTOPLAY };

    enum EntityTypes { SPACESHIP, MONSTER, BASIC_MONSTER, SHOT, BASIC_SHOT, BUTTON, WALL, BACKGROUND, ROOM, ROOMID, ROOMMODE, SHAPE, TEXT, TEXTBOX, TEXTBOXNUMBER, PSEUDO, MUSIC, SFX };
} // namespace ecs
