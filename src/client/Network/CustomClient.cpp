    ...
    switch (msg.header.id)
    {
        ...
        case network::CustomMessage::AllGameComponentSent:
        {
            ...
            for (auto &it : registry->getEntities()) {
                rect = sprites_manager->get_Animations_rect(registry->getComponents<ecs::Type>().at(it).value().getEntityType(),
                registry->getComponents<ecs::Type>().at(it).value().getEntityID(), 0);
...
void CustomClient::receivedGameComponent(network::Message<network::CustomMessage> &msg)
{
    switch (_actualType)
    {
