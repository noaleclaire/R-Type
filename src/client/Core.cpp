...
Core::Core(boost::asio::io_context &io_context, std::string host, unsigned short server_port) :
    _io_context(io_context), _client(io_context, host, server_port)
{
    _client.all_data_received = &_all_data_received;
...
void Core::_switchScenes()
{
    if (_last_scene != ecs::Scenes::GAME && Core::actual_scene == ecs::Scenes::GAME) {
        _client.initGame();
        while (!_all_data_received);
        _all_data_received = false;
    }
